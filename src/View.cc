...
int* View::getChannels()
{
	int channelsValues[3] = { this->channels[0], this->channels[1], this->channels[2]};
	int *pointer = channelsValues;
	return channels;
}


void View::setChannels(int* channels)
{
	for (int i = 0 ; i < 3 ; i++)
	{
...
void View::setProcessArgs(int* processArgs)
{
	for (int i = 0 ; i < 4 ; i++)
	{
		this->process_args[i] = processArgs[i];
	}
}
